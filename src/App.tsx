...
marked.setOptions({
  breaks: true,
  gfm: true, // GitHub Flavored Markdown - allows HTML
});
...
function App() {
  ...
  // Subscribe to dice engine result cards
  useEffect(() => {
    const unsubscribe = subscribeToResultCards((card) => {
      setResultCards((prev) => [card, ...prev]);

      // If logging to entry is enabled, add the card to the active entry
      if (uiSettings.logToEntry && activeEntryId) {
        const html = renderResultCardHtml(card);

        // Update draft content
        setActiveEntryDraftContent((prevDraft) => {
          const separator = prevDraft.trim().length ? "\n\n" : "";
          return `${prevDraft}${separator}${html}`;
        });
    ...
    <div className="app-root">
      <div className="app-shell" style={{ gridTemplateColumns }}>
        ...
        <aside className="app-sidebar" style={{ width: leftPane.visible ? leftPane.width : 0, display: leftPane.visible ? "flex" : "none" }}>
          ...
          <div className="app-sidebar-toolbar">
            ...
            <button
              className={`sidebar-toolbar-button icon-button${searchOpen ? " sidebar-toolbar-button-active" : ""
                }`}
            ...
            <div className="app-tools-expanded">
              <div className="app-tools-header">
                <div className="app-tools-tabs">
                  <button
                    className={`tool-tab icon-button${activeTool === "results" ? " tool-tab-active" : ""
                      }`}
                  ...
                  <button
                    className={`tool-tab icon-button${activeTool === "dice" ? " tool-tab-active" : ""
                      }`}
                  ...
                  <button
                    className={`tool-tab icon-button${activeTool === "tables" ? " tool-tab-active" : ""
                      }`}
                  ...
                  <button
                    className={`tool-tab icon-button${activeTool === "diceDev" ? " tool-tab-active" : ""
                      }`}
                  ...
                  {settings.developerMode && (
                    <button
                      className={`tool-tab icon-button${activeTool === "devTools" ? " tool-tab-active" : ""
                        }`}
      ...
      <footer className="app-status-bar">
        <div className="app-status-actions">
          ...
          <button
            className={`status-icon-button icon-button${uiSettings.logToEntry ? " sidebar-icon-button--active" : ""
              }`}
